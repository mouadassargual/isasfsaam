...
const scheduleData = [
  {
    day: 1, date: "8 Dec", dateEn: "Dec 8", dayNameFr: "Lundi", dayNameEn: "Monday",
    theme: "Opening & Project Building", themeFr: "Ouverture & Construction de Projet",
    emoji: "🚀", color: "bg-blue-500", gradient: "from-blue-500 to-cyan-500",
    events: [
      ...
      { time: "11:15 - 12:45", titleEn: "Smart Systems Needs by Local Government (Agadir) & Private Sector (Haliopole)", titleFr: "Besoins en systèmes intelligents - Ville d'Agadir & Haliopole", type: "conference", location: "Salle des conférences" },
  ...
  {
    day: 2, date: "9 Dec", dateEn: "Dec 9", dayNameFr: "Mardi", dayNameEn: "Tuesday",
    ...
    events: [
      { time: "10:00 - 11:00", titleEn: "Build AI Solutions", titleFr: "Construire des Solutions IA", speaker: "Pr El Mehdi Cherrat", type: "keynote", location: "Salle des conférences" },
      { time: "11:00 - 11:15", titleEn: "Break & Speed Networking", titleFr: "Pause & Speed Networking", type: "networking", location: "Hall" },
      { time: "11:15 - 12:45", titleEn: "Industrial Smart Solutions", titleFr: "Solutions Industrielles Intelligentes", speaker: "Mohamed El Habib Abaakil", type: "keynote", location: "Salle des conférences" },
      { time: "14:00 - 17:00", titleEn: "Co-Creating Smart Solutions", titleFr: "Co-création de solutions intelligentes", speaker: "Mohamed El Habib Abaakil", type: "workshop", location: "Salle Polyvalente" },
    ],
  },
  {
    day: 3, date: "10 Dec", dateEn: "Dec 10", dayNameFr: "Mercredi", dayNameEn: "Wednesday",
    ...
    events: [
      ...
      { time: "11:00 - 11:15", titleEn: "Break & Speed Networking", titleFr: "Pause & Speed Networking", type: "networking", location: "Hall" },
      { time: "11:15 - 12:45", titleEn: "Building Information Modeling (BIM) & Virtual Reality", titleFr: "BIM & Réalité Virtuelle", speaker: "Ismail Khoubbaz", type: "keynote", location: "Salle des conférences" },
  ...
  {
    day: 4, date: "11 Dec", dateEn: "Dec 11", dayNameFr: "Jeudi", dayNameEn: "Thursday",
    theme: "Agriculture & Data Pipeline", themeFr: "Agriculture & Pipeline de Données",
    emoji: "🌾", color: "bg-primary", gradient: "from-primary to-orange-500",
    events: [
      { time: "10:00 - 11:00", titleEn: "Robotic Precision Agriculture: Sensors, Drones, and Innovations", titleFr: "Agriculture de précision robotique: Capteurs, Drones et Innovations", speaker: "Pr Amine Saddik", type: "keynote", location: "Salle des conférences" },
      { time: "11:00 - 11:15", titleEn: "Break & Speed Networking", titleFr: "Pause & Speed Networking", type: "networking", location: "Hall" },
      { time: "11:15 - 12:45", titleEn: "To Be Confirmed", titleFr: "À confirmer", type: "tba", location: "Salle des conférences" },
      { time: "14:00 - 17:00", titleEn: "Building a Modern Open-Source Data Pipeline", titleFr: "Pipeline de données open-source moderne", speaker: "Nabil Ayoub & Anas Bennis", type: "workshop", location: "Salle Polyvalente" },
    ],
  },
  {
    ...
    events: [
      { time: "10:00 - 11:00", titleEn: "Keynote Session (TBC)", titleFr: "Session Keynote (À confirmer)", speaker: "Pr Aimad Karkouch", type: "keynote", location: "Salle des conférences" },
      { time: "11:00 - 11:15", titleEn: "Break & Speed Networking", titleFr: "Pause & Speed Networking", type: "networking", location: "Hall" },
      { time: "11:15 - 17:00", titleEn: "Systems Integration Hackathon: From Sensor to Dashboard", titleFr: "Hackathon: Du Capteur au Dashboard", speaker: "Zakaria Oulad", type: "hackathon", location: "Salle des conférences" },
          ...
          {scheduleData.map((day, index) => (
            <button
              ...
              className={cn(
                ...
                expandedDay === index
                  ? `bg-gradient-to-br ${day.gradient} text-white shadow-2xl scale-105 z-10`
                  : "bg-card/60 backdrop-blur-md border border-border/50 hover:border-primary/30"
              )}
            >
              {expandedDay === index && <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent" />}
              <div className="relative">
                <div className="text-3xl mb-2">{day.emoji}</div>
                <div className={cn("text-2xl font-black", expandedDay !== index && "text-foreground")}>
                  {language === "en" ? day.dateEn.split(" ")[1] : day.date.split(" ")[0]}
                </div>
                <div className={cn("text-xs font-medium uppercase tracking-wider", expandedDay === index ? "text-white/80" : "text-muted-foreground")}>
                  {language === "en" ? day.dayNameEn : day.dayNameFr}
                </div>
                <div className={cn("text-xs mt-2 line-clamp-1", expandedDay === index ? "text-white/90" : "text-muted-foreground")}>
          ...
          {scheduleData.map((day, index) => (
            <button
              ...
              className={cn(
                ...
                expandedDay === index
                  ? `bg-gradient-to-br ${day.gradient} text-white shadow-xl`
                  : "bg-card/60 backdrop-blur-md border border-border/50"
              )}
            >
              <span className="text-2xl mb-1">{day.emoji}</span>
              <span className="text-xl font-black">{language === "en" ? day.dateEn.split(" ")[1] : day.date.split(" ")[0]}</span>
              <span className="text-[10px] uppercase tracking-wider opacity-80">{language === "en" ? day.dayNameEn.slice(0, 3) : day.dayNameFr.slice(0, 3)}</span>
            </button>
          ))}
        </div>
            ...
            <div key={day.day} className="relative">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={cn("w-14 h-14 rounded-xl flex items-center justify-center text-2xl", expandedDay === dayIndex ? "bg-white/20" : "bg-secondary/50")}>
                    {day.emoji}
                  </div>
                  <div className="text-left">
                    <div className="flex items-center gap-3">
                      <span className={cn("text-xl font-black", expandedDay !== dayIndex && "text-foreground")}>
                        {language === "en" ? `${day.dayNameEn} ${day.dateEn}` : `${day.dayNameFr} ${day.date}`}
                      </span>
                      <Badge className={cn("border-0 text-xs", expandedDay === dayIndex ? "bg-white/20 text-white" : `bg-gradient-to-r ${day.gradient} text-white`)}>
                        {day.events.length} {language === "en" ? "events" : "événements"}
                      </Badge>
                    </div>
                    <p className={cn("text-sm mt-1", expandedDay === dayIndex ? "text-white/80" : "text-muted-foreground")}>
                      {language === "en" ? day.theme : day.themeFr}
                    </p>
                  </div>
                </div>
                <ChevronDown className={cn("w-6 h-6 transition-transform duration-300", expandedDay === dayIndex ? "rotate-180" : "", expandedDay !== dayIndex && "text-muted-foreground")} />
              </button>

              <div className={cn("overflow-hidden transition-all duration-500", expandedDay === dayIndex ? "max-h-[2000px] opacity-100 mt-4" : "max-h-0 opacity-0")}>
                <div className="relative pl-8 md:pl-12">
                  <div className={cn("absolute left-3 md:left-5 top-0 bottom-0 w-1 rounded-full bg-gradient-to-b", day.gradient)} />
                    ...
                    {day.events.map((event, eventIndex) => {
                      ...
                      return (
                        <div key={eventIndex} className="relative group">
                          <div className={cn("absolute -left-5 md:-left-7 top-5 w-4 h-4 rounded-full border-4 border-background z-10", isLive ? "bg-red-500 animate-pulse ring-4 ring-red-500/30" : `bg-gradient-to-br ${config.gradient}`)} />
                          <Card className={cn("overflow-hidden transition-all duration-300 group-hover:shadow-xl group-hover:scale-[1.01]", "bg-card/80 backdrop-blur-md border-border/30 hover:border-primary/30", isLive && "ring-2 ring-red-500/50 bg-red-500/5")}>
                            <CardContent className="p-0">
                              <div className="flex flex-col sm:flex-row">
                                <div className="sm:w-32 p-4 flex sm:flex-col items-center sm:items-start justify-center gap-2 border-b sm:border-b-0 sm:border-r border-border/30 bg-gradient-to-br from-secondary/30 to-transparent">
                                  <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", `bg-gradient-to-br ${config.gradient} text-white`)}>
                                    {config.icon}
                                  </div>
                                  <div>
                                    <div className="text-sm font-bold text-foreground">{event.time}</div>
                                    {isLive && <div className="flex items-center gap-1 text-xs text-red-500 font-bold mt-0.5"><Radio className="w-3 h-3 animate-pulse" />LIVE</div>}
                                  </div>
                                </div>
                                <div className="flex-1 p-4">
                                  <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                                    ...
                                    <Badge className={cn("shrink-0 border-0 text-white text-xs", `bg-gradient-to-r ${config.gradient}`)}>
                                  ...
                                  <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                                    ...
                                    {event.speaker && (
                                      <button onClick={(e) => { e.stopPropagation(); scrollToSpeaker() }} className="flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-primary/10 to-cyan/10 hover:from-primary/20 hover:to-cyan/20 transition-all border border-primary/20">
                                        {(() => {
                                          const info = getSpeakerInfo(event.speaker)
                                          if (info?.image) return <div className="w-5 h-5 rounded-full overflow-hidden ring-1 ring-primary/30"><Image src={info.image} alt={event.speaker} width={20} height={20} className="w-full h-full object-cover" /></div>
                                          if (info?.initials) return <div className="w-5 h-5 rounded-full bg-gradient-to-br from-primary to-cyan flex items-center justify-center"><span className="text-[8px] font-bold text-white">{info.initials}</span></div>
                                          return <User className="w-3.5 h-3.5 text-primary" />
                                        })()}
                                        <span className="text-xs font-medium text-primary hover:underline">{event.speaker.split(" (")[0]}</span>
          ...
          {[
            ...
            <Card key={i} className="bg-card/60 backdrop-blur-md border-border/50 overflow-hidden group hover:border-primary/30 transition-all hover:scale-105">
              <CardContent className="p-5 text-center relative">
                ...
                <div className={cn("w-12 h-12 rounded-xl mx-auto mb-3 flex items-center justify-center text-white bg-gradient-to-br", stat.gradient)}>{stat.icon}</div>
        ...
        <div className="mt-12 text-center">
          <Card className="max-w-xl mx-auto bg-gradient-to-r from-primary/10 via-card/80 to-cyan/10 border-primary/20 overflow-hidden">
            <CardContent className="p-6 md:p-8">
              ...
              <h3 className="text-xl font-bold text-foreground mb-2">{language === "en" ? "Full Program Available" : "Programme Complet Disponible"}</h3>
              <p className="text-muted-foreground text-sm mb-4">{language === "en" ? "December 8-12, 2025 • FSA Agadir" : "8-12 Décembre 2025 • FSA Agadir"}</p>
              <Button asChild className="bg-gradient-to-r from-primary to-cyan hover:opacity-90 border-0 text-white">
                <a href={GOOGLE_FORM_URL} target="_blank" rel="noopener noreferrer">{language === "en" ? "Register Now" : "S'inscrire Maintenant"}<ChevronRight className="w-4 h-4 ml-1" /></a>
              </Button>
            </CardContent>
          </Card>
        </div>
